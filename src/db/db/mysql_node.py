...
class MysqlNode(Node):
    def __init__(self):
        super().__init__("mysql_node")

        self.db_host = '192.168.1.65'
        self.db_port = 3306
        self.db_user = 'pinky'
        self.db_password = 'pinky1234'
        self.db_name = 'pinky_db'
        ...
        self.get_logger().info(f"Mysql Node initialized. Connecting to {self.db_host}:{self.db_port}/{self.db_name}")
    ...
    def connect_to_database(self):
        ...
        try:
            ...
            self.get_logger().info("Successfully connected to Mysql database")
            self.publish_status("connected")
            return True
        except Exception as e:
            self.get_logger().error(f"Failed to connect to Mysql: {str(e)}")
