...
# 수정 후
PLACE_Z_DROP_FIRST = 0   # 첫 번째 박스 하강량 (mm) — 튜닝 필요
PLACE_Z_DROP = 50.0  # 테이블까지 내려가는 상대 하강량 (mm) — 튜닝 필요
...
PLACE_J2_STEP   = 50.0  # 박스마다 J2 증가량 (도) — 작게 시작해서 튜닝
PLACE_MAX_COUNT = 3      # 최대 나열 개수

PLACE_J4_BASE  = -35.0 # 첫 번째 박스 J5 각도 (기준값 — 실측 후 입력)
PLACE_J4_STEP  = 0   # J2 한 스텝마다 J5 증가량 — 작게 시작해서 튜닝
...
class PickAndPlace:
    ...
    def _place(self, approach_z: float):
        ...
        #     raise RuntimeError("플레이스 Z 충돌 위험")
                ...
                # 수정 후
        if PLACE_Z_DROP > current_z_after:
            raise RuntimeError(
                f"하강량({PLACE_Z_DROP:.1f}mm)이 현재Z({current_z_after:.1f}mm)보다 큼 — 충돌 위험"
            )

        logger.info(
            f"플레이스 좌표: x={place_x:.1f}  y={place_y:.1f}  "
            f"현재Z={current_z_after:.1f}  드롭Z={PLACE_Z_DROP:.1f}  "
            f"하강량={current_z_after - PLACE_Z_DROP:.1f}mm"
        )

        # 5) 하강량 결정 — 첫 번째 박스는 별도 값 사용
        logger.info(f"드롭 하강 시작: 현재Z={current_z_after:.1f}  목표Z={PLACE_Z_DROP:.1f}mm")
        # _send_coords(self.mc, place_x, place_y, PLACE_Z_DROP, DESCEND_SPEED)
        
        # 수정 후
        drop_amount = PLACE_Z_DROP_FIRST if self._place_count == 0 else PLACE_Z_DROP
        ...
        logger.info(
            ...
            f"({'첫번째' if self._place_count == 0 else f'{self._place_count+1}번째'})"
        )
        ...
        if coords_drop:
            logger.info(f"드롭 후 실제 Z: {coords_drop[2]:.1f}mm  (목표={PLACE_Z_DROP:.1f}  차이={coords_drop[2]-PLACE_Z_DROP:.1f}mm)")
            print(f"\n  드롭 실제 Z: {coords_drop[2]:.1f}mm  목표: {PLACE_Z_DROP:.1f}mm  차이: {coords_drop[2]-PLACE_Z_DROP:.1f}mm")
