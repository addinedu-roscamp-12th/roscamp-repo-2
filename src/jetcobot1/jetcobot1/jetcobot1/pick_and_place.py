...
class PickAndPlace:
    ...
    def run(self, align_timeout: float = 30.0) -> bool:
        ...
        try:
            ...
            # run() Step 2 끝부분, _pick() 호출 전
            box = self.tracker.get_best_box()
            print(f"\n [디버그] box={box}") # ← 무조건 찍히는 줄
            if box is not None and box.angle is not None:
                print(f"\n  박스 각도    : {box.angle:.1f}°")  # ← 추가
                angles = self.mc.get_angles()
                if angles and len(angles) >= 6:
                    target_angles = angles[:]
                    target_angles[5] += box.angle
                    print(f"  그리퍼 각도 정렬: J6 +{box.angle:.1f}°")
                    self.mc.send_angles(target_angles, MOVE_SPEED)
    ...
    def _place(self, approach_z: float):
        ...
        """
        # 1) 현재 XY에서 안전 높이 상승 (LIFT_MARGIN만큼 추가 확보)
        coords = _get_coords(self.mc)
        if coords is None:
        ...
        _send_coords(self.mc, cur_x, cur_y, lift_z, DESCEND_SPEED)
        _wait_move(self.mc, timeout=5.0)
        time.sleep(0.3)
