...
REF_DISTANCE = 150.0    # 기준 카메라~박스 거리 mm (측정 후 입력)
...
CAMERA_GRIPPER_Y_OFFSET = 30.0
MOVE_CAMERA_DOWN = [0,  -35,  0,  -55,  0,  -47 ] 

# ────────────────────────────────────────────────────────────────
# 플레이스 설정
# ────────────────────────────────────────────────────────────────
PLACE_J1_ANGLE   = 100  # 플레이스 방향 J1 목표 각도 (도)
PLACE_J1_SPEED   = 30     # J1 회전 속도 — 박스 떨어지지 않게 느리게

# 각 박스 위치별 절대 좌표 (mm) — J2로 방향 잡은 후 이 좌표로 정확히 착지
# 측정법: 로그에서 각 위치의 x, y 평균값 입력 / z는 테이블 절대 Z 고정
PLACE_COORDS_LIST = [
    [50.0,   270.0, 250.0],  # 1번째 (J2=-35°)
    [33.0,   230.1, 207.0],  # 2번째 (J2=-25°)
    [50.5,    71.8, 207.0],  # 3번째 (J2=-15°)
]
...
PLACE_J2_START  = -35.0  # 첫 번째 박스 J2 각도 (카메라 하향 자세 기준)
PLACE_J2_STEP   = 10.0  # 박스마다 J2 증가량 (도) — 작게 시작해서 튜닝
PLACE_MAX_COUNT = 5      # 최대 나열 개수

PLACE_J4_BASE  = -35.0 # 첫 번째 박스 J4 각도 (기준값 — 실측 후 입력)
PLACE_J4_STEP  = -30   # J2 한 스텝마다 J4 증가량 — 작게 시작해서 튜닝
...
class PickAndPlace:
    ...
    플레이스 흐름:
        ...
        J2로 대략적인 방향 잡기
              ↓
        PLACE_COORDS_LIST 절대 좌표로 정확히 착지 → 그리퍼 열기 → 상승
    """
    ...
    def _place(self, approach_z: float):
        ...
        플레이스 시퀀스 (J1 회전 방식):
          ...
          3. J2로 대략적인 방향 잡기
          4. PLACE_COORDS_LIST 절대 좌표로 정확히 착지 → 그리퍼 열기 → 상승
        """
        ...
        # 3) 절대 좌표로 정확히 착지 — J2 자세와 무관하게 항상 같은 위치
        time.sleep(0.3)
        place_target = PLACE_COORDS_LIST[self._place_count % PLACE_MAX_COUNT]
        place_x = place_target[0]
        place_y = place_target[1]
        place_z = place_target[2]

        logger.info(
            f"플레이스 절대 좌표 이동: x={place_x:.1f}  y={place_y:.1f}  z={place_z:.1f}  "
            f"(#{self._place_count + 1}번째)"
        )
        _send_coords(self.mc, place_x, place_y, place_z, DESCEND_SPEED)
        _wait_move(self.mc, timeout=5.0)
        time.sleep(0.2)
        ...
        if coords_drop:
            logger.info(f"드롭 후 실제 Z: {coords_drop[2]:.1f}mm  (목표={place_z:.1f}  차이={coords_drop[2]-place_z:.1f}mm)")
            print(f"\n  드롭 실제 Z: {coords_drop[2]:.1f}mm  목표: {place_z:.1f}mm  차이: {coords_drop[2]-place_z:.1f}mm")
