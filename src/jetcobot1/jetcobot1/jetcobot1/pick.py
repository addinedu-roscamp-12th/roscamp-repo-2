...
REF_AREA     = 34926.5  # 기준 픽셀 면적 (측정 후 입력)
...
MOVE_CAMERA_DOWN = [0,  -35,  0,  -55,  0,  -47 ] 

# ────────────────────────────────────────────────────────────────
# 플레이스 설정
# ────────────────────────────────────────────────────────────────
PLACE_J1_ANGLE   = 105  # 플레이스 방향 J1 목표 각도 (도)
PLACE_J1_SPEED   = 30     # J1 회전 속도 — 박스 떨어지지 않게 느리게

# PLACE_Z_DROP     = 260  # 테이블 윗면 Z (mm) — 실측값
# 수정 후
# 각 박스 위치별 하강량 (mm) — J2 각도마다 팔 끝 높이가 달라지므로 위치별로 측정 후 입력
# 측정법: 각 위치에서 로그의 "드롭 후 실제 Z" 확인 후 테이블까지 남은 거리 추가
PLACE_Z_DROP_LIST = [
    0.0,    # 1번째 (J2=-55°): 테이블에 닿음 확인됨
    70.0,   # 2번째 (J2=-15°): 로그 기준 계산값 (튜닝 필요)
    65.0,    # 3번째 (J2=+25°): 실행 후 측정
]
PLACE_Z_APPROACH = 270  # 플레이스 접근 높이 (PLACE_Z_DROP + 80mm)

PLACE_X_OFFSET   = 100.0   # J1 회전 후 X축으로 추가 이동 거리 (mm) — 더 깊이 놓으려면 늘림
LIFT_MARGIN      = 50.0   # 픽업 후 회전 전 상승 여유 (mm) — 장애물 여유있게 높이면 늘림
LIFT_X_RETRACT = +50.0 # X 후퇴 거리 (mm) — 음수: 베이스 방향, 양수: 반대 방향

PLACE_J2_START  = -55.0  # 첫 번째 박스 J2 각도 (카메라 하향 자세 기준)
PLACE_J2_STEP   = 30     # 박스마다 J2 증가량 (도) — 작게 시작해서 튜닝
PLACE_MAX_COUNT = 3      # 최대 나열 개수

PLACE_J4_BASE  = -35.0 # 첫 번째 박스 J5 각도 (기준값 — 실측 후 입력)
PLACE_J4_STEP  = -30   # J2 한 스텝마다 J5 증가량 — 작게 시작해서 
...
def estimate_z_distance(area: float) -> float:
    ...
    if area > REF_AREA:
        ...
        # 기준보다 작으면 덜 내려가도록 보정
        bonus = (REF_AREA - area) / REF_AREA * 67.0
        distance -= bonus
...
class PickAndPlace:
    ...
    def _pick(self, pick_x: float, pick_y: float,
        ...
        # 3-1) 그리퍼 방향을 박스 각도에 맞춰 J6 회전 (하강 직전)
        # box_angle: minAreaRect grip_angle (-90~90°)
        # J6 범위 클램프: -175° ~ 175°
    ...
    def _place(self, approach_z: float):
        ...
        """
        logger.info(f"[place] _place_count={self._place_count}  J2 목표={PLACE_J2_START + (self._place_count % PLACE_MAX_COUNT) * PLACE_J2_STEP:.1f}°")

        # 1) 원점 자세로 이동 (박스 들고) — J6는 파지 각도 유지
        logger.info("원점 자세로 이동 (경유)")
        current_angles = self.mc.get_angles()
        zero_with_j6 = list(ANGLES_ZERO)
        zero_with_j6[5] = current_angles[5]  # J6만 현재 각도 유지
        ...
        # # 가장 먼 쪽(J2 낮음) → 가까운 쪽(J2 높음) 순으로 나열
        # target_angles[1] = PLACE_J2_START + (self._place_count % PLACE_MAX_COUNT) * PLACE_J2_STEP
        # logger.info(
        #     f"J1={PLACE_J1_ANGLE}°  "
        #     f"J2={target_angles[1]:.1f}°  "
        #     f"(#{self._place_count + 1}/{PLACE_MAX_COUNT})"
        # )
        ...
        # 2-2) J2 나열 각도 + J5 높이 보정 적용  ← 여기
        angles = self.mc.get_angles()
        if not angles or len(angles) < 6:
            raise RuntimeError("J2 각도 읽기 실패")
        target_angles = angles[:]
        j2_target = PLACE_J2_START + (self._place_count % PLACE_MAX_COUNT) * PLACE_J2_STEP
        target_angles[1] = j2_target
        target_angles[3] = PLACE_J4_BASE + (self._place_count % PLACE_MAX_COUNT) * PLACE_J4_STEP
        logger.info(
            f"J2={j2_target:.1f}°  J4={target_angles[3]:.1f}°  "
            f"(#{self._place_count + 1}/{PLACE_MAX_COUNT})"
        )
        self.mc.send_angles(target_angles, PLACE_J1_SPEED)
        _wait_move(self.mc, timeout=6.0)
        ...
        # # 3) 카메라 하향 자세 경유 (J1은 회전한 각도 유지)
        # angles = self.mc.get_angles()
        # camera_down_angles = list(ANGLES_CAMERA_DOWN)
        # camera_down_angles[0] = PLACE_J1_ANGLE
        # camera_down_angles[1] = -15.0 # J2 조정 — 더 높이 들기
        # camera_down_angles[5] = angles[5]  # J6 현재 각도 유지
        # logger.info(f"카메라 하향 자세 경유: J1={PLACE_J1_ANGLE:.1f}° 유지")
        # self.mc.send_angles(camera_down_angles, 50)
        # # time.sleep(1.0)
        ...
        place_x = coords_after[0]
        place_y = coords_after[1]
        current_z_after = coords_after[2]  # ← J2 변화 후 실제 Z

        # PLACE_Z_DROP은 테이블 윗면 절대 Z값이므로 그대로 사용
        # 단, 현재 Z가 PLACE_Z_DROP보다 낮으면 위험하므로 체크
        # if current_z_after < PLACE_Z_DROP:
        #     logger.warning(
        #         f"현재 Z({current_z_after:.1f}) < PLACE_Z_DROP({PLACE_Z_DROP}) "
        #         f"— 테이블 충돌 위험, 스킵"
        #     )
        #     raise RuntimeError("플레이스 Z 충돌 위험")

        # 5) 위치별 하강량 결정
        drop_amount = PLACE_Z_DROP_LIST[self._place_count % PLACE_MAX_COUNT]
        target_drop_z = current_z_after - drop_amount

        logger.info(
            f"플레이스 좌표: x={place_x:.1f}  y={place_y:.1f}  "
            f"현재Z={current_z_after:.1f}  하강량={drop_amount:.1f}  "
            f"목표Z={target_drop_z:.1f}mm"
        )
        logger.info(
            f"드롭 하강: 현재Z={current_z_after:.1f}  "
            f"하강량={drop_amount:.1f}mm  "
            f"목표Z={target_drop_z:.1f}mm  "
            f"(#{self._place_count + 1}번째)"
        )
        _send_coords(self.mc, place_x, place_y, target_drop_z, DESCEND_SPEED)
        _wait_move(self.mc, timeout=5.0)
        time.sleep(0.2)

        # 실제로 어디까지 내려갔는지 확인
        coords_drop = _get_coords(self.mc)
        if coords_drop:
            logger.info(f"드롭 후 실제 Z: {coords_drop[2]:.1f}mm  (목표={target_drop_z:.1f}  차이={coords_drop[2]-target_drop_z:.1f}mm)")
            print(f"\n  드롭 실제 Z: {coords_drop[2]:.1f}mm  목표: {target_drop_z:.1f}mm  차이: {coords_drop[2]-target_drop_z:.1f}mm")
        ...
        # 6-1) 박스 건드리지 않도록 수직으로 먼저 상승
        logger.info("박스 이탈 상승 (각도 유지)")
        lift_angles = self.mc.get_angles()
        if lift_angles and len(lift_angles) >= 6:
            lift_angles[1] = lift_angles[1] + 40.0  # J2 올리면 팔 끝이 위로 올라감
            self.mc.send_angles(lift_angles, DESCEND_SPEED)
            _wait_move(self.mc, timeout=5.0)
            time.sleep(0.2)
...
def main():
    ...
    try:
        while True:
            ...
            # 성공/실패 무관하게 카메라 하향 자세 복귀
            print("카메라 하향 자세로 복귀 중...")
            setup.mc.send_angles(ANGLES_CAMERA_DOWN, ROBOT_SPEED)
            time.sleep(2.5)
